...
WNDCLASS NewWindowChildClass(HBRUSH BGColor, HCURSOR Cursor, HINSTANCE hInst, HICON Icon, LPCWSTR Name, WNDPROC Procedure);
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR args, int ncmdshow) {
	...
	HWND hWnd = CreateWindow(L"MainWndClass", L"Ввод матрицы", WS_OVERLAPPEDWINDOW | WS_VISIBLE, 100, 100, 500, 500, NULL, NULL, NULL, NULL);
...
LRESULT CALLBACK SoftwareChildProcedure(HWND childWindow, UINT msg, WPARAM wp, LPARAM lp) {
	...
	case WM_CREATEchild:
		break;
	case WM_CLOSE:
		EnableWindow(GetParent(childText), TRUE);
		if (MessageBox(childWindow, L"Закрытие этого окна приведет к закрытию приложения", L"Подтверждение", MB_OKCANCEL) == IDOK) {
